...
def writeImageAsGeoTIFF(img, transform, metadata, crs, filename, shard_dir):
    metadata.update({"driver":"GTiff",
                     ...
                     "transform": transform,
                     "crs": crs})
    with rasterio.open(shard_dir1+filename+".tif", "w", **metadata) as dest1:
        dest1.write(img)

    with rasterio.open(shard_dir+filename+".jpg", "w", **metadata) as dest:
        dest.write(img)
...
def get_geojson_detections1(tiff_path, shard_dir, outpath):
  with rasterio.open(tiff_path) as src:
      ...
      # 打印文件的保存的地址
  	print(shard_dir)
      # 调用裁剪图像，将原始的SAR图像信息裁剪640*640的图像，并且
  	splitImageIntoCells(src, "shard", 640, shard_dir)
...
def get_geojson_detections(tiff_path, shard_dir, outpath):
  ...
  for image_fp in tqdm(shard_list):
    ...
    if coords_list:
        mypath=image_fp[:-4]
        substrings = mypath.split("/")
        # 获取最后一个子串
        last_substring = substrings[-1]
        # 以"_"为分隔符，将最后一个子串拆分成多个子串
        final_substrings = last_substring.split("_")
        # 获取第一个子串，即"106"
        result = final_substrings[1]
        draw_bounding_boxes(im, coords_list, labels=None, save_path=predict_save_fold+"detect"+result+".jpg")
...
get_geojson_detections1(tiff_filepath, shard_dir,output_geojson_filepath)


# python my_main_detect.py subset_0_of_VH.tif detections1.geojson 0.5